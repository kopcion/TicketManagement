...
public class DbEngine {
    ...
    public static void login(String login, String password) throws LoginException {
        ...
        try {
            PreparedStatement stmt = connection.prepareStatement("insert into last_logins values(?)");
            stmt.setInt(1, getID(login));
            stmt.execute();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static ObservableList getTickets(String user, String search){
        ...
        try {
            ...
            for(int i=0; i < number; i++){
                if (set.getString(1).contains(search) || set.getString(2).equals(search)) lol.add(new TicketDetails(set.getString(1), set.getString(2), set.getString(3), 0));
                set.next();
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static ObservableList getLastLogs(String user){
        ObservableList list = FXCollections.observableArrayList();
        try {
            Statement statement = connection.createStatement();
            ResultSet set = statement.executeQuery("SELECT * FROM last_logins join logins on IDUser = id where '" + user + "' = username;");
            int number = countRows("last_logins join logins on IDUser = id", " username = '"+user+"'" );
            for(int i=0; i < number; i++){
                set.next();
                list.add(set.getTimestamp(2));
                System.out.println(set.getString(1));
            }
        } catch (SQLException e) {
            e.printStackTrace();
